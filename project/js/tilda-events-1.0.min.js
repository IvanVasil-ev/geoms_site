                    ...
                    ((t = t.split('#')), (d = d + '#' + t[1]), (t = t[0])),
                      ...
                      document.getElementById('tildaredirectform')
                        ? (document.getElementById('tildaredirectform').setAttribute('method', 'GET'),
                          document.getElementById('tildaredirectform').setAttribute('action', d))
    ...
    ;(Tilda.sendEcommerceEvent = function (e, t) {
      ...
      (Tilda.sendEventToStatistics = function (e, t, r, a) {
        var i = '/' === e.substring(0, 1),
          d = []
        ;(g = document.getElementById('allrecords')) &&
        (m =
          null !== g.getAttribute('data-fb-event')
            ? g.getAttribute('data-fb-event')
            : void 0 !== $('#allrecords').data('fb-event')
              ? $('#allrecords').data('fb-event')
              : null)
        var c = !(!m || 'nosend' !== m),
          o = g ? g.getAttribute('data-vk-event') : null
        g &&
        (o =
          null !== g.getAttribute('data-vk-event')
            ? g.getAttribute('data-vk-event')
        ...
        var n,
          ...
          l = g.getAttribute('data-tilda-currency')
            ? g.getAttribute('data-tilda-currency')
        ...
        try {
          if (window.gtagTrackerID && 'gtag' === window.mainTracker)
            if (i)
              if (r && r.event)
                if ('purchase' === r.event) {
                  for (var d = r.ecommerce.purchase.products, s = 0, p = 0; p < d.length; p++)
                    if ('delivery' === d[p].id) {
                      s = d[p]
                      break
                    }
              ...
              else gtag('config', window.gtagTrackerID, { page_title: t, page_path: e })
            ...
            if (r && r.event) {
              try {
                if (
                  ...
                  for (var v = r.ecommerce.purchase.products.length, p = 0; p < v; p++)
                    (f = r.ecommerce.purchase.products[p]),
                      ga('ec:addProduct', {
                        id: f.id || p,
                        name: f.name,
                  ...
                  for (v = r.ecommerce.add.products.length, p = 0; p < v; p++)
                    (f = r.ecommerce.add.products[p]),
                      ga('ec:addProduct', {
                        id: f.id || p,
                        name: f.name,
                        price: parseFloat(f.price).toFixed(2),
                  ...
                  for (v = r.ecommerce.remove.products.length, p = 0; p < v; p++)
                    (f = r.ecommerce.remove.products[p]),
                      ga('ec:addProduct', {
                        id: f.id || p,
                        name: f.name,
                        price: parseFloat(f.price).toFixed(2),
                  ...
                  for (v = r.ecommerce.detail.products.length, p = 0; p < v; p++)
                    (f = r.ecommerce.detail.products[p]),
                      ga('ec:addProduct', {
                        id: f.id || p,
                        name: f.name,
                        price: parseFloat(f.price).toFixed(2),
        ...
        if (
          ...
          (i
            ? ((n = { title: t }),
            0 < a && ((n.params = { order_price: a }), l && (n.params.currency = l)),
              ...
              if (
                ...
                if (r && r.event && 0 < a)
                  if ('addToCart' === r.event && r.ecommerce.add) {
                    for (var v = r.ecommerce.add.products.length, w = [], p = 0; p < v; p++)
                      (f = r.ecommerce.add.products[p]), w.push(f.id || f.uid || f.name)
                    ...
                    for (v = r.ecommerce.detail.products.length, w = [], p = 0; p < v; p++)
                      (f = r.ecommerce.detail.products[p]), w.push(f.id || f.uid || f.name)
        ...
        if (void 0 !== window.VK && void 0 !== window.VK.Retargeting && !1 == m)
          try {
            if (i) {
              var g,
                f,
                h = (g = document.getElementById('allrecords'))
                  ? g.getAttribute('data-vk-price-list-id')
                  : null,
                y = (h && parseInt(h)) || 0,
                h = '',
                k = !1
              if (r && r.event)
                if (
                  ((k = { products: [], currency_code: '', total_price: 0 }),
                  'purchase' === r.event && r.ecommerce.purchase)
                )
                  if (0 < a && 0 < y) {
                    k.currency_code = l
                    for (v = r.ecommerce.purchase.products.length, w = [], p = 0; p < v; p++)
                      (f = r.ecommerce.purchase.products[p]),
                        k.products.push({ id: f.id || f.uid || f.name, price: f.price || 0 }),
                        (k.total_price = a)
                    h = 'init_checkout'
                  } else h = 't-purchase'
                else if ('addToCart' === r.event && r.ecommerce.add)
                  if (0 < a && 0 < y) {
                    k.currency_code = l
                    for (v = r.ecommerce.add.products.length, w = [], p = 0; p < v; p++)
                      (f = r.ecommerce.add.products[p]),
                        k.products.push({ id: f.id || f.uid || f.name, price: f.price || 0 }),
                        (k.total_price = a)
                    h = 'add_to_cart'
                  ...
                  if (0 < a && 0 < y) {
                    ...
                    (h += '-' + r.ecommerce.detail[0].uid)
                else if ('removeFromCart' === r.event && r.ecommerce.remmove)
                  if (0 < a && 0 < y) {
                    k.currency_code = l
                    for (v = r.ecommerce.remove.products.length, w = [], p = 0; p < v; p++)
                      (f = r.ecommerce.remove.products[p]),
                        k.products.push({ id: f.id || f.uid || f.name, price: f.price || 0 }),
                        (k.total_price = a)
                    h = 'remove_from_cart'
                    ...
                    (h += '-' + r.ecommerce.remove[0].uid)
                else h = r.event
              else
                h =
                  -1 !== e.indexOf('tilda/') && -1 !== e.indexOf('/payment/')
                    ? 't-purchase-' +
                    e.replace('/tilda/', '').replace('tilda/', '').replace('/payment/', '')
                    : -1 !== e.indexOf('tilda/') && -1 !== e.indexOf('/submitted/')
                      ? 't-lead-' +
                      e.replace('/tilda/', '').replace('tilda/', '').replace('/submitted/', '')
                      : -1 !== e.indexOf('tilda/popup') || -1 !== e.indexOf('tilda/click')
                        ? 't-' + e.replace('/tilda/', '').replace('/', '-')
                        : 't-' + e.replace('/', '-')
              0 < y && k && k.currency_code
                ? (VK.Retargeting.Event('purchase'), VK.Retargeting.ProductEvent(y, h, k))
                : (VK.Retargeting.Event(h),
                  't-purchase' === h.substring(0, 10)
                    ? VK.Goal('purchase')
                    : 't-lead' === h.substring(0, 6) && VK.Goal('lead'))
        ...
        ((m = window._tmr || (window._tmr = [])),
          i
            ...
            : 0 < a
              ? m.push({ id: '' + window.mainMailruId, type: 'reachGoal', goal: e, value: a })
              : m.push({ id: '' + window.mainMailruId, type: 'reachGoal', goal: e })),
        ...
        try {
          ...
          if (
            ...
            ((t = new Date()).setDate(t.getDate() + 30),
              (document.cookie =
                'TILDAUTM=' + encodeURIComponent(r) + '; path=/; expires=' + t.toUTCString()))
          }
        } catch (e) {}
      }),
